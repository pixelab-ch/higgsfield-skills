...
- [ ] **Phase 1: Shared Infrastructure + Repo Hygiene** — Build the load-bearing foundation every skill depends on: shared/model-catalog.md, shared/generation-flow.md, shared/mcp-tools.md, LICENSE, and clean repo state (remove zh-CN, audit frontmatter)
- [ ] **Phase 2: Pilot Skill (01-cinematic) End-to-End** — Validate the lean SKILL.md template, progressive disclosure structure, FR variant pattern, and the full opt-in generation loop on one skill before multiplying it across 14 more
...
- [ ] **Phase 4: Install Script + FR i18n Polish** — Revise install.sh to target the correct shared Claude Code + Desktop path, handle shared/ directory, add --fr/--all/--list/--target flags, idempotency; verify all 15 FR SKILL.md files
- [ ] **Phase 5: Spec Accuracy Pass + Documentation** — Re-verify every model-specs.md against live models_explore, update verification dates, and rewrite README.md + README.fr.md to reflect the real structure
...
**Success Criteria** (what must be TRUE):
  ...
  3. All 14 skills have `fr/SKILL.md` variants with `-fr` suffixed `name:` values, mirroring their EN counterparts
  4. All 14 skills have fully populated `references/model-specs.md` files sourced from `models_explore` data (no fabricated resolution, format, or fps claims), annotated with verification date
  5. Running any of the 14 skills delivers the same confirmation-gated generation loop as the pilot: prompt → confirm panel → explicit YES → generate → job report
**Plans**: TBD

### Phase 4: Install Script + FR i18n Polish
**Goal**: The install script works correctly for both Claude Code and Claude Desktop, installs the shared/ directory so relative references resolve, supports FR variants with the --fr flag, and all 15 FR SKILL.md files pass a quality check
**Depends on**: Phase 3
**Requirements**: PKG-01, PKG-02, PKG-03, PKG-04, I18N-02, I18N-03
**Success Criteria** (what must be TRUE):
  1. `install.sh` installs skills to `~/.claude/skills/` (not the old Desktop-only path) and correctly copies `shared/` to `~/.claude/shared/` so `../../shared/` relative links resolve from installed skill directories
  2. The installer supports `--all`, `--list`, `--fr`, and `--target code|desktop|project` flags; running `./install.sh --all` twice in a row produces no errors and no duplicate installs (idempotent)
  ...
  4. All 15 `fr/SKILL.md` files have >50% French body lines, unique `name:` values ending in `-fr`, reference EN references/ files correctly, and contain no zh-CN remnants
**Plans**: TBD

### Phase 5: Spec Accuracy Pass + Documentation
**Goal**: Every per-model spec in every skill is verified against the live models_explore catalog as of this pass, and README.md + README.fr.md accurately describe the real repo structure with no phantom files or stale statistics
**Depends on**: Phase 4
...
**Success Criteria** (what must be TRUE):
  ...
  2. `README.md` (EN) and `README.fr.md` (FR) accurately state the skill count (15), correct install paths (`~/.claude/skills/`), real output constraints sourced from model-catalog.md, and reference no files that do not exist
